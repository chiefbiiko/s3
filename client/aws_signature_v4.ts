...
export function kdf(
  secretAccessKey: string | Uint8Array,
  ...
  if (typeof secretAccessKey === "string") {
    secretAccessKey = encode(secretAccessKey, keyInputEncoding) as Uint8Array;
  }
  ...
  const paddedKey: Uint8Array = new Uint8Array(4 + secretAccessKey.byteLength);

  paddedKey.set(AWS4, 0);
  paddedKey.set(secretAccessKey, 4);
